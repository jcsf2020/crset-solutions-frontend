...
if (!(Test-Path $statusPath)) {
  Set-Content $statusPath "# Outreach - $day`nTargets: RemoteOK, WeWorkRemotely, Agency-PT, Agency-ES, Spare`nGoal: 5 applications today (Next.js + FastAPI)`n`n## Submissions`n- [ ] RemoteOK -`n- [ ] WeWorkRemotely -`n- [ ] Agency-PT -`n- [ ] Agency-ES -`n- [ ] Spare -`n`n## Notes`n"
}
...
Add-Content $notePath "`n---`nSubmitted: $(Get-Date -Format s)Z`nRole: $role`nCompany: $company`nURL: $url"
...
$replacement = "$day,$platform,$role,$company,$url,applied,"
...
Write-Host "Recorded: $platform / $role / $company"
