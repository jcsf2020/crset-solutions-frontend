...
function Card({ plan }: { plan: Plan }) {
  return (
    <div className={`card p-6 md:p-7 flex flex-col justify-between relative ${plan.recommended ? "ring-1 ring-black/10" : ""}`} aria-label={`Plano ${plan.name}`}>
      {plan.recommended && (
        <div className="absolute -top-3 right-4 rounded-full border border-black/10 bg-white px-3 py-1 text-xs shadow">Recomendado</div>
      )}
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-2">
        ...
        <a href="https://wa.me/351000000000?text=Quero%20demo%20CRSET" target="_blank" rel="noopener noreferrer" className="btn-secondary px-4 py-2 text-center">Falar no WhatsApp</a>
      </div>
    ...
    <section className="py-14 md:py-20" data-hide-sticky-cta>
      <div className="container-pro">
        <header className="text-center max-w-2xl mx-auto mb-10 md:mb-14">
          ...
          <p className="muted mt-3">Escolhe o plano certo para o teu momento. Upgrade facil quando estiveres pronto para escalar.</p>
        </header>
